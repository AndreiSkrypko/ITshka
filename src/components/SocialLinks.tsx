    ...
    <div className="flex items-center gap-2">
      {links.map((link, index) => (
        <a
          ...
          className={`group relative w-9 h-9 rounded-xl bg-gradient-to-br ${link.gradient} flex items-center justify-center transition-all duration-300 hover:scale-110 ${link.shadow}`}
          title={link.label}
          aria-label={link.label}
        >
          <link.icon className="w-4 h-4 text-white" />
          {/* Shine effect */}
          <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500 overflow-hidden" />
        </a>
      ))}
    </div>
  );
