    ...
    <div className="flex items-center gap-1.5 sm:gap-2">
      {links.map((link, index) => (
        <a
          ...
          className={`group relative w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl bg-gradient-to-br ${link.gradient} flex items-center justify-center transition-all duration-300 hover:scale-110 ${link.shadow} overflow-hidden`}
          title={link.label}
          aria-label={link.label}
        >
          <link.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white relative z-10" />
          {/* Shine effect */}
